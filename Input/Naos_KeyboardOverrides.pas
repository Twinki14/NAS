...
begin
   User32.PostMessage(Naos.getGameClient(), WM_KEYDOWN, key, MAKELPARAM(0, 1));
end;

procedure KeyUp(key: Word); override;
begin
   User32.PostMessage(Naos.getGameClient(), WM_KEYUP, key, MAKELPARAM(1, 0));
end;
...
begin
   ...
   begin
    ...
    if( (s[i]=Uppercase(s[i])) or special) then
       User32.PostMessage(Naos.getGameClient(), WM_CHAR, WPARAM(s[i]), MAKELPARAM(1, 0))
    else
      begin
