procedure TNaosPair.debug(fString :string; clear:boolean=false);
begin
  if(self.z_debug) then
  begin
    if clear then
      ClearDebug();
    fString:=formatDateTime('tt',time())+' | '+ 'Naos_Pair ' +' > ' +fString;
    WriteLn(fString);
  end;
end;

function TNaosPair.isActive(): boolean;
