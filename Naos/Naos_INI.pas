procedure TNaosINI.debug(fString :string; clear:boolean=false);
begin
  if(self.z_debug) then
  begin
    if clear then
      ClearDebug();
    fString:=formatDateTime('tt',time())+' | '+ 'Naos_INI ' +' > ' +fString;
    WriteLn(fString);
  end;
end;

function TNaosINI.GetINIKeys(section: string; maxCount: int32=128): array of Int32;
Var
  cnt: int32;
  str: String;
begin
...
function TNaosINI.ReadINIKeyValues(section: string): array of String;
Var
  i: integer;
...
function TNaosINI.GetLastINIKey(): Int32;
begin
  result:=high(self.ReadINIKeyValues(self.section))+1;
end;

function TNaosINI.ParseINIValues(): array of TNaosINIPair;
Var
  i: integer;
...
procedure TNaosINI.DeclareINI(iniName: String='Naos_Pairs.ini'; section: String='Paired_Clients'; path: String=IncludePath+'Naos\');
begin
  self.name := iniName;
...
procedure TNaosINI.WritePairToINI(pair: TNaosPair);
Var lKey: int32;
begin
...
procedure TNaosINI.CleanupINI();
Var
  i: int32;
  vA: Array of TNaosINIPair;
  rs: boolean;
begin
  vA:=parseINIValues();
  for i := low(vA) to high(vA) do
  begin
    if(Naos.isTIDActive(vA[i].Simba_TID))=false then
      begin
        rs:=true;
      end;
    if(Naos.isPIDActive(vA[i].Simba_PID))=false then
      begin
        rs:=true;
      end;
    if (Naos.isPIDActive(vA[i].NXT_PID))=false then
      begin
        rs:=true;
      end;
    if(rs) then
...
function TNaosINI.isPairedWithINI(pair: TNaosPair): boolean;
Var
  i: int32;
  vA: Array of TNaosINIPair;
  rs: boolean;
